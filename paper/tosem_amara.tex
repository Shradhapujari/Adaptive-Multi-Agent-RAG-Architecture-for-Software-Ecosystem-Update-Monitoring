...
(Section~\ref{sec:leak}). When a run is made in replay against a corpus
snapshot, the fetch is served from that snapshot, so the arms differ by
capability rather than by what the live feed happened to hold at the moment
each one ran.
...
state the pooling protocol with every absolute number for this reason.
...
Section~\ref{sec:results} names the run id it was computed from, and every
number in it is recoverable from that directory without re-running the live
endpoints.
...
the grading cascade on the multi-agent arm, that arm leads the baseline on
every rank-sensitive metric: nDCG@1 $+0.049$ $[+0.023, +0.077]$, nDCG@3
$+0.032$ $[+0.012, +0.053]$, MRR $+0.025$ $[+0.007, +0.044]$, each surviving a
Holm correction, with 94 wins to 63 losses at rank 3. Recall@5 does not move
($+0.014$, CI spanning zero), which is consistent with the mechanism: the
documents were already in the pool, and what changed is where they sit in it.
Faithfulness is indistinguishable from the baseline once answer format is
matched ($+0.001$).

That comparison on its own does not establish what produced the lead. The
grading cascade runs on the multi-agent arm and not on the baseline, which is
the treatment---it is the Evaluator reading what was retrieved instead of
counting its sources---but it is also 20 model calls the baseline does not
make, and a ranking gain from reading candidates is not obviously a gain from
decomposing the pipeline. Every earlier candidate for a multi-agent advantage
in this paper dissolved at exactly this point, under exactly this question.

\subsubsection{The Attributing Control}\label{sec:tier-control}

We therefore ran the baseline a second time with the grading cascade attached
to it, over the same 500 questions, in strict replay against the corpus
snapshot the previous run recorded---10{,}310 document reads served from disk
and no miss on any corpus host, so the two arms read the same documents by
construction. The arms are then identical in ranking function, corpus, judge,
and per-question model cost, and differ only in whether retrieval was
decomposed. Because the arms ran in separate runs, each with its own judged
pool, all three are re-scored against the union of both runs' judgments before
pairing; the union is what Table~\ref{tab:tier-control} reports, which shifts
the values of the preceding paragraph by at most 0.001.

\begin{table}[t]
\caption{The grading cascade given to each architecture in turn. $n=500$,
scored against the pooled judgments of both runs, paired over questions with
percentile bootstrap 95\% CI and Holm correction. The cascade does not help
the baseline: given the same 20 model calls per question, the single-agent arm
does not improve, and on recall it degrades. The same cascade on the
multi-agent arm's larger pool gains on every metric.}
\label{tab:tier-control}
\begin{tabular}{llrrrr}
\toprule
Arm & Ranking function & nDCG@1 & nDCG@3 & Recall@5 & MRR \\
\midrule
Single-agent & \texttt{flat:embed}      & 0.460 & 0.479 & 0.513 & 0.550 \\
Single-agent & \texttt{flat:llm20@embed} & 0.441 & 0.467 & 0.478 & 0.528 \\
Multi-agent  & \texttt{flat:llm20@embed} & \textbf{0.509} & \textbf{0.511} & \textbf{0.525} & \textbf{0.575} \\
\midrule
\multicolumn{2}{l}{Cascade on the baseline ($\Delta$ vs.\ row 1)}
  & $-0.019$ & $-0.012$ & $-0.035$ & $-0.022$ \\
\multicolumn{2}{l}{Multi-agent vs.\ baseline, both with the cascade}
  & $+0.068$ & $+0.044$ & $+0.047$ & $+0.047$ \\
\multicolumn{2}{l}{\quad 95\% CI}
  & $[{+}0.039,$ & $[{+}0.020,$ & $[{+}0.017,$ & $[{+}0.023,$ \\
\multicolumn{2}{l}{}
  & ${+}0.097]$ & ${+}0.069]$ & ${+}0.076]$ & ${+}0.071]$ \\
\multicolumn{2}{l}{\quad W/T/L}
  & 47/441/12 & 93/342/65 & 86/370/44 & 55/411/34 \\
\bottomrule
\end{tabular}
\end{table}

The control does not reproduce the gain. Given the same cascade and the same
cost, the single-agent arm does not improve on its own \texttt{flat:embed}
configuration: nDCG@3 $-0.012$ with a confidence interval spanning zero, and
recall@5 $-0.035$ with one that does not. Reading candidates is not, in
general, worth 20 model calls. It is worth them when there is something in the
pool to promote, which is the condition the union fetch creates and the
baseline's single-phrasing fetch does not.

Set side by side under one ranking function, one corpus, one judge, and one
cost, the multi-agent arm leads the single-agent baseline by $+0.044$ nDCG@3
$[+0.020, +0.069]$, $+0.068$ nDCG@1, $+0.047$ MRR, and---for the first time in
this paper---on recall, $+0.047$ $[+0.017, +0.076]$, winning 86 questions to
44. This is the one place where the decomposition pays, and the shape of the
result says why it took this long to find: the advantage was created at fetch
time by the union, held in the candidate pool through every measurement in
Sections~\ref{sec:two-metrics}--\ref{sec:leak}, and destroyed at ranking time
by the tier prior. Both halves had to be fixed before either was visible.

We hold this result to the same standard as the ones it replaces. The two arms
ran in separate runs rather than side by side in one, so the pairing rests on
the corpus being identical by construction rather than by design; a single run
carrying all three arms would be stronger. The judge remains
\texttt{llama3.1}, which shares a model family with the system under test
(Section~\ref{sec:threats}). And the gain is an ordering gain of 0.04--0.07 on
a metric whose absolute level is near 0.5, not a difference in kind: the
single-agent baseline remains a strong competitor at a third of the latency
and a twentieth of the model calls.
...
deployed system answering unrelated questions pays the cold price. nDCG@3 is
from the 500-question measurements of Section~\ref{sec:tier-control}, scored
against pooled judgments.}
\label{tab:cost}
\begin{tabular}{llrrrrr}
\toprule
...
\midrule
Single-agent & \texttt{flat:embed}       &  1 &  1.0 &   332 &   315 & 0.479 \\
Single-agent & \texttt{flat:llm20@embed} & 22 & 12.6 & 5{,}033 & 2{,}984 & 0.467 \\
Multi-agent  & \texttt{flat:embed}       &  3 &  3.0 & 1{,}199 & 1{,}105 & parity \\
Multi-agent  & \texttt{flat:llm20@embed} & 23 & 16.4 & 6{,}012 & 4{,}192 & \textbf{0.511} \\
\bottomrule
\end{tabular}
\end{table}
...
question, and the baseline cannot use them.} Given exactly that budget, the
single-agent arm goes from 0.479 to 0.467 nDCG@3 and loses 0.035 recall@5
(Table~\ref{tab:tier-control}). This is the clearest cost result in the paper
and the most easily missed one: the same 20 model calls per question are worth
nothing on one architecture and $+0.044$ nDCG@3 on the other, because what they
do is promote documents, and only one of the two arms fetched documents worth
promoting.
...
calls and 13 times the tokens for $+0.032$ nDCG@3 over the cheapest baseline,
or $+0.044$ over the baseline at matched cost. End-to-end latency in the
...
with flat ranking and an embedding reranker is the configuration to deploy: it
is within 0.032 nDCG@3 of the best result here at a sixteenth of the model
calls. The decomposition earns its cost only where the 0.03--0.04 matters
enough to pay an order of magnitude for it, and---on the evidence of the
control---only when the extra calls are spent reading a candidate pool that a
union fetch has already widened. Neither half is worth buying alone.
...
\begin{enumerate}
  ...
  \item With the prior removed, one capability of the decomposition pays. The
  Evaluator grading the top 20 retrieved documents instead of counting their
  sources gains nothing for the single-agent baseline given the same 20 model
  calls per question ($-0.012$ nDCG@3, and $-0.035$ recall@5), and gains on
  every metric for the multi-agent arm, whose union fetch put documents in the
  pool for it to promote. Under one ranking function, one corpus, one judge and
  one cost, the multi-agent arm leads the baseline by $+0.044$ nDCG@3
  $[+0.020, +0.069]$ and $+0.047$ recall@5 $[+0.017, +0.076]$ over 500
  questions (Table~\ref{tab:tier-control}). The advantage was created at fetch
  time and destroyed at ranking time; both had to be fixed before either was
  visible.
\end{enumerate}
