...
does not hold raises rather than quietly going live. The raise is not the
guarantee it first appears to be, and we correct our own earlier description of
it: each source fetch in the Retriever is individually guarded, so the
exception is caught there and that source returns no documents for that
question. A strict run does not stop on a miss; it loses a source silently and
reports the count at the end, which is where \texttt{frozen} in a run's
configuration comes from. A run whose \texttt{frozen} is false has holes of
this kind, and their number is the honest measure of how frozen it was. Either way the
...
harness does not persist the round count per question, so we counted the
questions on which the retry arm spent two rewriter invocations instead of one
(94, against the baseline's uniform one), and a run that logged the count
directly would be better. And 74 of the 30{,}808 document reads in the second
