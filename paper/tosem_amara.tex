...
measured here. The measurement it requires is a single frozen pass carrying the
retry arm and its comparison arm together: the two arms must share one judged
pool, because the demonstration in Section~\ref{sec:pool-demo} separates
byte-identical rankings by $+0.055$ recall@5 when they do not. We report the
rung as reachable and unmeasured, which is a different claim from the null it
could have been mistaken for, and we decline to predict which of the two the
measurement will return.
