...
every rank-sensitive metric: nDCG@1 $+0.049$ $[+0.022, +0.077]$, nDCG@3
$+0.033$ $[+0.012, +0.053]$, MRR $+0.025$ $[+0.007, +0.044]$, each surviving a
...
We state plainly what this comparison does and does not establish. The grading
cascade runs on the multi-agent arm and not on the baseline, which is the
treatment---it is the Evaluator reading what was retrieved instead of
counting its sources---but it is also 20
model calls the baseline does not make, and a ranking gain from reading
candidates is not obviously a gain from decomposing the pipeline. The
attributing comparison is the baseline given the same cascade, on the same
questions and the same recorded corpus.
%% TODO(control): run_1790117750 -- single_agent + flat + llm20@embed, n=500,
%% strict replay of corpus_snapshot_b500_flat_0921. Score with
%% scripts/cross_run_compare.py against the pooled qrels of run_1789976732.
%% If it lands near 0.486, the +0.033 is the multi-agent pool; if near 0.52,
%% the gain is the reranker and this subsection's claim must be withdrawn.
...
\begin{enumerate}
  ...
  \item One capability of the decomposition survives as a ranking gain rather
  than a claim: the Evaluator grading the top 20 retrieved documents instead of
  counting their sources is worth $+0.027$ nDCG@3 over the same flat ranking
  without it, at 20 model calls per question. With it, the multi-agent arm leads
  the baseline over 500 questions on nDCG@1, nDCG@3 and MRR, while recall is
  unchanged --- an ordering gain, not a fetching one (Section~\ref{sec:tier}).
\end{enumerate}
