...
\subsection{Language-Model Agents and Retrieval Augmentation in Software Engineering}
...
\subsection{Multi-Agent RAG Frameworks and Reproducibility Infrastructure}

Multi-agent RAG systems distribute retrieval and reasoning across specialised
roles. Four recent designs are close enough to the pipeline studied here to
warrant comparison --- Self-RAG's reflection tokens~\cite{asai2024selfrag},
CRAG's lightweight retrieval evaluator~\cite{yan2024crag}, Adaptive-RAG's
complexity-based routing~\cite{jeong2024adaptiverag}, and FLARE's
confidence-triggered retrieval~\cite{jiang2023flare} --- together with the
agentic-RAG systems surveyed more
broadly~\cite{singh2025agenticrag,mishra2025agenticsurvey,shaikh2025marag,zhang2025mainrag}.
Each decides \emph{whether} and \emph{how often} to retrieve and how to filter
what returns. None, as published, checks whether the text handed to the
retriever still carries the user's own phrasing, which is the failure this
paper reports.

Two toolkits make much of that work runnable: FlashRAG reimplements RAG
algorithms under one
interface~\cite{jin2025flashrag,flashrag2025repo}, and RAGLAB reproduces six
algorithms across ten benchmarks with published
checkpoints~\cite{zhang2024raglab,raglab2024repo}. We report no head-to-head
comparison against them, for a reason that is a property of this setting rather
than a criticism of the toolkits: each assumes a static corpus embedded and
indexed in advance, while the corpus here is a set of live endpoints whose
contents change between runs. A reimplementation of the self-reflective
mechanisms over our own retrieval ships with the harness and is described, with
the independence conditions its evaluation would have to satisfy, in
Section~S10 of the supplementary material; no results from it are reported.

\subsection{Corrective Retrieval, Adaptive Feedback, and Self-Improvement}
...
The measurement practice this paper depends on --- graded relevance judgments
pooled across systems, nDCG and its discounting, the recall/precision split a
rewrite sits astride, and the use of a language model as a judge together with
what that costs in independence --- is established work rather than a
contribution here. Section~S3 of the supplementary material reviews it and
names the conventions each number in Section~\ref{sec:results} follows.
...
Retrieval draws on three kinds of document through one provider's API:
vendor release notes and a vendor registry, CVE and advisory feeds, and
community discussion threads. Verified and community sources are kept apart
and weighted differently by the Evaluator. Section~S5 of the supplementary
material lists every endpoint, its trust tier, how each phrasing enters it,
and the entry counts measured when this was written.
...
The evaluation uses questions mined from real community threads across 24
software ecosystems, stratified by category and ecosystem, in sets of 100,
300, 500 and 1{,}000. Section~S4 of the supplementary material gives the
construction in full: the sources each question is drawn from, the category
and ecosystem strata, how ground-truth answers are assigned, and the rule that
makes a smaller set a strict prefix of a larger one so a limited run can be
resumed upward.
...
Two arms are comparable only when one capability separates them.
Table~\ref{tab:ladder} is built that way: each rung adds exactly one capability to the rung
beneath it, so a difference between adjacent rungs is attributable to that
capability and to nothing else. Section~S2 of the supplementary material gives
the full argument, including what makes two arms share a corpus, why a
record-mode run and a replay of it are not the same measurement, and the
pooling rule under which every number in Section~\ref{sec:results} is scored.
...
(Table~\ref{tab:ladder-results}); A4 executed but could not trigger as run,
for the structural reason given in Section~\ref{sec:retry}, which also reports
the fix that makes the rung reachable.}
\label{tab:ladder}
\begin{tabular}{llll}
\toprule
...
Table~S6 of the supplementary material reports it.
...
Table~S8 of the supplementary material is the most useful evidence in this section, for a reason
...
baseline's median here, twice it in Table~S7 of the supplementary material --- is the
quantity to read.

Table~S7 of the supplementary material carries a methodological result worth stating
...
Tables~\ref{tab:scaled-retrieval} to~S8 are now reported.
...
Table~S9 of the supplementary material reports them under a Wilcoxon signed-rank test.
...
by Section~S1 of the supplementary material its three arms share one corpus and its numbers
...
Section~S1 of the supplementary material those are not the same corpus: a snapshot holds one
...
The parity this table reports has since been measured again at twice the scale,
on the full 1{,}000-question benchmark, with flat ranking and an embedding
reranker on all three arms in one pass (run \texttt{run\_1790365310}, replayed
against a snapshot of 24{,}455 entries recorded in a preceding pass; that replay
was lenient and recorded its own misses, so the corpus is fixed for the arms
within the run while the run itself is not frozen). It is the same answer: nDCG@3 0.485 for both
multi-agent arms against 0.476 for the baseline, a paired $+0.009$ with a
confidence interval of $[-0.003, +0.021]$, and a win/tie/loss of 101/796/103 --
as close to a dead heat as 1{,}000 questions can express. nDCG@5 is $+0.010$
$[-0.003, +0.023]$ and recall@5 $+0.016$ $[-0.002, +0.033]$, neither surviving
correction. Doubling the sample moves the absolute level not at all and the gap
not at all, which is what a null looks like when it is not a power problem.

That measurement is reproducible in the strongest sense this paper can offer. A
strict replay of the same snapshot (run \texttt{run\_1790415950}), executed from
a revision in which a document missing from the snapshot aborts the run rather
than silently emptying a source, returned the same retrieved set for all 3{,}000
arm-question pairs and the same metrics to five decimal places, against 51{,}245
document reads and no miss on any document host. One qualification on what that
demonstrates: the two runs share the accumulated relevance cache, so the metric
agreement is partly the same judgments being reused rather than independently
re-derived. The document-level agreement is the part that is independent, and it
is also the stronger claim, since identical retrieved sets are what make the
judgments reusable at all.

The same run reproduces the rendering artifact at full size, and we note it here
because scale is the usual defence against it. The template arm scores $0.165$
higher on correctness than the baseline on the 139 questions carrying a
reference answer ($p<0.001$), while the same retrieval rendered as prose scores
$0.004$ \emph{lower} than the baseline and is indistinguishable from it; the
template arm is simultaneously $0.063$ below the baseline on faithfulness
($p<0.001$) where the prose arm is level. A judge that rewards a structured
answer on one axis and penalizes it on another, for content it did not change,
is measuring the rendering on both. At $n=1000$ the effect is sharper rather
than weaker, which is the outcome Section~\ref{sec:metric-threat} predicts and
the reason that section retires the comparison rather than seeking a larger
sample for it.
...
harness once carried. What those retries do is now measured, in the one
design the question admits: a two-pass protocol over the 500 questions, the
first pass recording the retry arm alone so that its second-round rewrites'
fetches --- phrasings no earlier snapshot could contain --- are captured, the
second pass replaying both arms against that snapshot in a single run with one
judged pool. Replaying one arm while the other fetched live would have left the
retry arm reading a fresher corpus than the arm it is compared against, which
is the confound of Section~\ref{sec:tier-control} at a smaller scale; a
recording pass made fresh puts the whole corpus on one date instead of
documenting the asymmetry.

\begin{table}[t]
\caption{The retry measured. 500 questions, both arms in one pass with one
judged pool, replayed against a snapshot recorded by the retry arm itself
(run \texttt{run\_1790278467}, pass 2 of two; 30{,}734 document reads served
from disk and 74 not in the snapshot, for which the affected source returned
nothing). The retry fired on 94 questions and changed the top-4 on 71 of them.
Every metric is flat.}
\label{tab:retry}
\begin{tabular}{lrrrl}
\toprule
Metric & Retry & No retry & $\Delta$ (95\% CI) & W/T/L \\
\midrule
...
Table~\ref{tab:retry} reports it. The retry fired on 94 of 500 questions, a
rate of 18.8\% against the 13\% the offline projection gave over the older
top-4 lists, and it was not idle when it fired: the retrieved set changed on 71
of those 94 questions and the answer on 42 of the 500. None of it moves a
metric. Every retrieval difference is within $0.004$ with an interval
straddling zero and a win/loss split as close to even as the counts allow ---
8 better and 11 worse at rank 3, 6 and 8 on recall. Faithfulness and answer
relevance move $+0.002$, which is nothing.

This is the result the rung has been unable to state in either direction since
the conference version, and its shape matters more than its sign. The
Orchestrator's second round is neither inert nor cosmetic here: it asks the
Query Rewriter for vocabulary the first round did not try, fetches on it, and
brings back a different set of documents for one question in five. Those
documents are, on average, neither better nor worse than the ones they
displaced. Adaptive retrieval as this system implements it re-rolls the dice
rather than improving the odds, and a re-roll is what a null of this shape
looks like: motion without direction.

Two limits on the claim. The fire rate is inferred rather than recorded: the
harness did not persist the round count per question when this run was made, so
we counted the questions on which the retry arm spent two rewriter invocations
instead of one (94, against the baseline's uniform one). A run that logs the
count is the better instrument, and reconstructing it from token accounting is
the kind of inference that should not be load-bearing twice. And 74 of the 30{,}808 document reads in the second
pass were not in the snapshot, so the affected source returned nothing for
those questions; at 0.24\% of reads that adds noise rather than a direction,
but the pass is not the fully frozen article its protocol was meant to
produce.
...
by role and excluding the judge (Section~S1 of the supplementary material); each
...
\begin{enumerate}
  ...
  surviving correction (Table~\ref{tab:tier-control}); repeated on all 1{,}000
  benchmark questions the answer is $+0.009$ nDCG@3, 101 won and 103 lost. An earlier comparison
