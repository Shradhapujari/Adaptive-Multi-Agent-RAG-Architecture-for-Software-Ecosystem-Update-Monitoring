...
Run:
    python evaluate_v2.py --quick    # 5 queries
    python evaluate_v2.py            # 15 queries
...
SYNONYMS = {
    "linux":     ["linux","ubuntu","debian","kernel","fedora","redhat","distro"],
    "security":  ["security","vulnerability","cve","patch","exploit","advisory","threat"],
    "bug":       ["bug","defect","issue","error","crash","fault","regression","fix"],
    "fix":       ["fix","patch","resolve","resolved","hotfix","bugfix","correction"],
    "update":    ["update","upgrade","release","version","changelog","patch"],
    ...
    "community": ["community","reddit","users","developers","reaction","feedback"],
    "reaction":  ["reaction","response","feedback","opinion","sentiment","complaint"],
    "release":   ["release","version","changelog","notes","published","shipped"],
    "cve":       ["cve","vulnerability","exploit","security","advisory","nvd"],
    ...
    "vscode":    ["vscode","visual studio","editor","microsoft","ide"],
}
...
EXPANSIONS = {
    ...
    "reaction": "community sentiment feedback opinion response",
    "negative": "negative community reaction frustrated complaint",
    "notes":    "release notes changelog version update",
}

TEST_QUERIES = [
    {"query":"Any critical Linux security patches today","category":"security","expected_keywords":["linux","security","patch","vulnerability","cve","kernel"]},
    ...
    {"query":"Chrome browser crash fix latest version","category":"bugs","expected_keywords":["chrome","crash","fix","bug","browser","resolved"]},
    {"query":"VSCode debugger bug fixed","category":"bugs","expected_keywords":["vscode","debugger","bug","fix","resolved"]},
    {"query":"Latest Grafana release notes","category":"releases","expected_keywords":["grafana","release","notes","version","update","monitoring"]},
    ...
    {"query":"How do users feel about latest MacOS update","category":"community","expected_keywords":["macos","update","users","community","reaction","feedback"]},
    ...
    {"query":"Which software releases got negative feedback","category":"community","expected_keywords":["software","release","negative","feedback","community","reaction"]},
    {"query":"Any critical software updates today","category":"general","expected_keywords":["software","update","critical","release","security","patch"]},
]
...
def fetch_releases(query, limit=8):
    try:
        ...
        for v in all_v:
            ...
            overlap = len(exp & set(txt.split()))
            if overlap>0: scored.append((overlap,v))
        scored.sort(key=lambda x:x[0],reverse=True)
        return [v for _,v in scored[:limit]]
    except: return []

def fetch_community_api(query, limit=8):
    try:
        ...
        for t in terms:
            ...
            for base,syns in SYNONYMS.items():
                if t in syns or t==base: exp.update(syns)
        scored = [(len(exp & set((p.get("title","")+p.get("subreddit","")).lower().split())),p) for p in all_p]
        scored = sorted([(s,p) for s,p in scored if s>0],key=lambda x:x[0],reverse=True)
        return [p for _,p in scored[:limit]]
    except: return []

def fetch_community_local(query, limit=8):
    terms = set(query.lower().split())
    exp = set()
    for t in terms:
        exp.add(t)
        for base,syns in SYNONYMS.items():
            if t in syns or t==base: exp.update(syns)
    scored = [(len(exp & set((p["title"]+p["subreddit"]).lower().split())),p) for p in LOCAL_POSTS]
    scored = sorted([(s,p) for s,p in scored if s>0],key=lambda x:x[0],reverse=True)
    return [p for _,p in scored[:limit]]
...
def run_baseline(qobj):
    r = fetch_releases(qobj["query"],3)
    c = fetch_community_local(qobj["query"],3)
...
def run_marag(qobj, mem):
    ...
    r = fetch_releases(rw,8)
    c = fetch_community_api(rw,5)
    if not c: c = fetch_community_local(rw,5)
    cv = fetch_cve(query,3)
    q = score(r,c,cv,qobj["expected_keywords"])
    retried = False
    if sig(q)=="negative":
        r2=fetch_releases(query+" software update security patch release fix",8)
        c2=fetch_community_local(query+" software update security patch release",5)
...
def report(br,ar,curve,mem):
    ...
    print(f"  Learning gain       : +{lc_imp}% (early→late queries)")
    print(f"\n  Per category:")
...
if __name__=="__main__":
    ...
    print(f"\n  Local dataset: {len(LOCAL_POSTS)} posts | Mode: {'QUICK' if quick else 'FULL'}")
